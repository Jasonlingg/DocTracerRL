...
if [[ -n "${RESEARCH_PAPER_RERANKER_MODEL:-}" ]]; then
  ...
  reranker_args+=(--paper-reranker-model "$RESEARCH_PAPER_RERANKER_MODEL")
  reranker_args+=(--paper-reranker-revision "$RESEARCH_PAPER_RERANKER_REVISION")
  reranker_args+=(--paper-reranker-device "${RESEARCH_PAPER_RERANKER_DEVICE:-cpu}")
fi
